...
const getDashboard = async (req, res, next) => {
  try {
    ...
    // ── Negative alert emails (non-blocking, once per month per alert type) ──
    const userEmail = req.user.email;
    const userName = req.user.name;
    const monthLabel = new Date().toLocaleString('en-IN', { month: 'long', year: 'numeric' });

    if (monthlySavings < 0 && shouldSendAlert(userId, 'monthly_savings')) {
      sendEmail({
        to: userEmail,
        subject: `⚠️ Alert: Negative Monthly Savings – ${monthLabel}`,
        html: `
          <div style="font-family:sans-serif;max-width:520px;margin:auto">
            <div style="background:#ef4444;padding:24px;border-radius:12px 12px 0 0;text-align:center">
              <h2 style="color:white;margin:0">⚠️ Negative Savings Alert</h2>
            </div>
            <div style="background:#ffffff;padding:28px;border:1px solid #e2e8f0;border-radius:0 0 12px 12px">
              ...
              <p style="color:#475569;font-size:14px">💡 <strong>Tip:</strong> Review your expense categories and cut down on non-essential spending to get back on track.</p>
