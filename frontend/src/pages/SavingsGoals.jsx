  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-main">Savings Goals</h1>
      ...
      {loading ? <LoadingSkeleton rows={3} /> : goals.length === 0 ? (
        <Card><p className="text-muted text-sm text-center py-12">No savings goals yet. Create one to get started!</p></Card>
      ) : (
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${g.is_completed ? 'bg-emerald-500/20' : 'bg-indigo-500/20'}`}>
                    ...
                    {g.is_completed && (
                      <span className="text-xs bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded-full font-medium">Achieved!</span>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold text-main">{pct.toFixed(0)}%</span>
                    </div>
                    <div className="w-full bg-elevated rounded-full h-2.5 overflow-hidden">
                      <div className={`h-2.5 rounded-full transition-all duration-700 ${getProgressColor(pct)}`}
                        style={{ width: `${pct}%` }} />
                    </div>
                  ...
                  <div className="flex gap-2 mt-4">
                    ...
                    <button onClick={() => openEdit(g)} className="p-2 text-muted hover:text-main hover:bg-hover rounded-lg transition-colors">
                      <Pencil className="w-4 h-4" />
                    </button>
                    <button onClick={() => handleDelete(g.id)} className="p-2 text-muted hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors">
      ...
      <Modal open={contributeModal} onClose={() => setContributeModal(false)} title={`Add to "${selectedGoal?.title}"`}>
        <form onSubmit={handleContribute} className="space-y-4">
          <div className="p-4 bg-elevated rounded-xl text-center">
            ...
            <p className="text-2xl font-bold text-main mt-1">{formatCurrency(selectedGoal?.current_amount || 0)}</p>
            <p className="text-xs text-muted mt-1">of {formatCurrency(selectedGoal?.target_amount || 0)}</p>
          </div>
